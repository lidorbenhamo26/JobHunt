...
2. Login wall - use your judgment, preferring Google SSO:
   ...
   - The portal already has an account for {ACCOUNT_EMAIL} from an earlier application? Sign in to it the same way (Google SSO) and reuse its saved profile - just verify/refresh every field against the data sources.
   - SSO callback blocked (ERR_BLOCKED_BY_CLIENT / a blank or "blocked" page after choosing the account) = a browser extension (AdBlock) killed the redirect, not a login problem. Retry once; if it happens again try, in this order: (a) the same portal's other SSO buttons that use an ALREADY signed-in session (LinkedIn / Microsoft) - never one that asks for a password; (b) "continue as guest" / "apply without an account" / an apply flow that only needs an email address (no password); (c) if none exists -> finish with: LOGIN_REQUIRED: AdBlock blocked the SSO callback on <portal> - allowlist <domain> in AdBlock, then continue. Name the exact blocked URL.
...
5. A REQUIRED question not answered verbatim in the data sources: answer it YOURSELF when the truthful answer is clearly inferable from them (standard eligibility like "are you 18+", years of experience with a technology, willingness/availability questions covered by application-answers.md, "how did you hear about us", start date / notice period, work authorization - application-answers.md has all of these; salary too, use its range or single number exactly as written there). Stop with NEEDS_INPUT: <the question> ONLY when you cannot answer truthfully from the data sources - a legal declaration, security clearance, or a company-specific fact that application-answers.md doesn't cover. Optional unanswered fields: leave blank. A dropdown with no exact match: pick the closest truthful option and mark it [inferred] in the review summary.
   AUTONOMY RULE: the user wants this to run without them. Before stopping with any status other than SUBMITTED/READY_FOR_REVIEW, ask yourself "can I truthfully resolve this from the data sources, the page itself, or a retry?" - if yes, do it. Stop only for things that are impossible for you: a password/2FA, a CAPTCHA, a blocker in the browser, or a fact that exists nowhere in the data sources.
